...
def realtime_vis(model, imgs):
    cmap = plt.get_cmap('tab20b')
    colors = [cmap(i) for i in np.linspace(0, 1, 20)]


    fig = plt.figure(figsize=(len(imgs) * 2.5, 3), dpi=100)
    num=0
    for img in imgs:
        num+=1
        ax = fig.add_subplot(1, len(imgs), num, xticks=[], yticks=[])
        img_pad=preprocess_img(img,[800,1333])
...
if __name__=="__main__":
    ...
    model_dir="./checkpoint/model_30.pth"
    model=FCOSDetector(mode="inference",config=Config)
    # model=torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)
    # print("INFO===>success convert BN to SyncBN")
    model = torch.nn.DataParallel(model)
    model.load_state_dict(torch.load(model_dir,map_location=torch.device('cpu')))
    # model.load_state_dict(torch.load("./checkpoint/batchsize8版/model_14.pth",map_location=torch.device('cpu')))
